...
# predicted species at specific sites.  

library("spectre")
library("foreach")

# set parameters 
replicate <- 1:10 # replicates per parameter combination
...
sim_fun <- function(siminputrow, parameters, writeRDS, verbose) 
{
  ...
  if (n_available_species >= n_species){
    ...
    if (known_sites > 0){
      ...
      res_min_conf <- spectre::run_optimization_min_conf_0(alpha_list = alpha_list, # check wheter both algorithms are the same here!
                                                           total_gamma = total_gamma, 
                                                           target = target_commonness, 
                                                           max_iterations = max_iterations,
                                                           energy_threshold = energy_threshold,
    ...
    saveRDS(r_1, file = paste0("./BCI_res_known_sites/", siminputrow, ".rds")) 
  }
}
