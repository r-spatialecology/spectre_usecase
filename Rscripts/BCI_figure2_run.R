...
# Create error ~ iteration plot data (for Figure 2)

out_dir <- "data/figure2"
...
sim_fun <- function(siminputrow, parameters, writeRDS, verbose)  
{
  ...
  if(!( dim(p)[1] == 1)) { # only used for quick testing 
    p <- parameters[1, ] 
  }
  ...
  if (n_available_species >= n_species){
    ...
    # MAE_c & RCE (unneccessary for Figure 2)
    solution_commonness <- spectre:::calculate_solution_commonness_rcpp(res_min_conf$optimized_grid)
    sum(abs(solution_commonness - target_commonness), na.rm = TRUE)
    MAE_c <- mean(abs(solution_commonness - target_commonness), na.rm = TRUE) 
    mean_target_commonness <- mean(abs(target_commonness), na.rm = TRUE)
    RCE <- MAE_c / mean_target_commonness * 100 
    ...
    r_1 <- tibble::tibble(replicate = replicate,
                          ...
                          n_species = n_species,
                          MAE_c = MAE_c,
                          mean_target_commonness = mean_target_commonness,
                          RCE = RCE,
                          iterations = max_iterations,
                          time = calc_time,
                          branch = out_dir)
    
    saveRDS(r_1, file = paste0(out_dir, "/BCI100Sites100SpeciesForPlotting.rds"))
