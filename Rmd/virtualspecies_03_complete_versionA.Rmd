---
title: "Spectre virtual species: Version A"
author: "Jan Salecker"
date: "8/31/2020"
...
## Spectre virtual species: Version A
...
In this example (Version A) we vary four factors to investigate how spectre scales:

* landscape size
* spatial autocorrelation of presences within species (spatial clustering)
...
#### STORE RESULTS:
saveRDS(results, file=file.path(dir.cloud, "data", "virtualspecies_03_complete_versionA.rds"))
...
library(tidyverse)
sapply(list.files("../R", full.names = TRUE), source)
results <- readRDS("../data/virtualspecies_03_complete_versionA.rds")
plotdir <- "../figures/virtualspecies_complete_versionA"
...
```{r plot.rec}
ggplot(results_rce, aes(x=factor(corr_within), y=factor(corr_among), fill=rce)) +
  facet_grid(landscape_size~gamma) +
  geom_tile() +
...
```
```{r plot.mae}
ggplot(results_rce, aes(x=factor(corr_within), y=factor(corr_among), fill=mae)) +
  facet_grid(landscape_size~gamma) +
  geom_tile() +
...
```{r plot.mc}
ggplot(results_rce, aes(x=factor(corr_within), y=factor(corr_among), fill=mc)) +
  facet_grid(landscape_size~gamma) +
  geom_tile() +
